using System.Diagnostics;
using System.Net;
using Newtonsoft.Json.Linq;
using System;
using System.Collections.Generic;

namespace Code2014Registration
{
    class PrefixClass
    {
        static void Prefix()
        {
            var url = "http://challenge.code2040.org/api/prefix";

            dynamic jsonObject = new JObject();
            jsonObject.token = "3d886e47f8b097899fb925d7cfb5a00c";

            string responsetxt = "";
            using (var client = new WebClient())
            {
                client.Headers[HttpRequestHeader.ContentType] = "application/json";
                responsetxt = client.UploadString(url, "POST", jsonObject.ToString());
            }

            Debug.WriteLine("RESPONSE: " + responsetxt);

            dynamic responsejson = JObject.Parse(responsetxt);

            string prefix = responsejson.prefix;

            JArray strarray = responsejson.array;
            JArray newarray = new JArray();
            foreach (string word in strarray)
            {
                if (!word.StartsWith(prefix))
                {
                    newarray.Add(word);

                }

            }

            jsonObject.array = newarray;


            string response2 = "";
            using (var client2 = new WebClient())
            {
                client2.Headers[HttpRequestHeader.ContentType] = "application/json";
                response2 = client2.UploadString("http://challenge.code2040.org/api/prefix/validate", "POST", jsonObject.ToString());
            }

            Debug.WriteLine("RESPONSE2: " + response2);


        }

        //static void Main()
        //{
        //    Prefix();
        //}
    }
}
